...
import (
	...
	"github.com/TIBCOSoftware/honeycomb/flogo-contrib/extension/utils"
...
func (a *LedgerActivity) Eval(context activity.Context) (done bool, err error) {
	...
	if stubobj == nil {
		return false, fmt.Errorf("containerServiceStub is not initialized")
	}

	stub, ok := stubobj.(shim.ChaincodeStubInterface)
	if !ok {
		return false, fmt.Errorf("containerServiceStub is not instance of shim.ChaincodeStubInterface")
	}
	...
	for _, av := range inputs {
		...
		if operation == "DELETE" {
			...
			rawvalue, err := utils.GetRecord(assetName, assetKey, asset, stub)
			if err != nil {
				return false, err
			}
			if rawvalue != nil {
			...
			//LOOKUP
			records, err := lookupRecords(assetName, assetKey, lookupKey, asset, stub)
			if err != nil {
				return false, err
			}
...
func createSecondaryCompositeKeys(assetName string, keys string, record map[string]interface{}, stub shim.ChaincodeStubInterface) ([]string, error) {
	...
	for _, key := range keyarray {
		compkey, _, err := utils.CreateCompositeKey(record, utils.GetSecondaryCompositeNS(assetName), key, stub)
		if err != nil {
			return nil, err
		}
...
func deleteRecord(assetName string, identifier string, input map[string]interface{}, compositeKeys interface{}, stub shim.ChaincodeStubInterface) ([]byte, error) {
	key, keyref, err := utils.CreateCompositeKey(input, assetName, identifier, stub)
	if err != nil {
		return nil, err
	}
...
func lookupRecords(assetName string, identifier string, lookupKey interface{}, input map[string]interface{}, stub shim.ChaincodeStubInterface) ([][]byte, error) {
	result := make([][]byte, 0)
	keyvalues, err := utils.CreatePartialCompositeKey(input, lookupKey.(string), stub)
	if err != nil {
		return nil, err
	}
	...
	if identifier != lookupKey.(string) {
		...
		namespace = utils.GetSecondaryCompositeNS(assetName)
	}
