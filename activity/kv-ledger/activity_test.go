...
func TestEval(t *testing.T) {
	...
	operation := "PUT"
	isArray := false
	containerServiceStub := shim.NewMockStub("IOU", &TestChainCode{})
	containerServiceStub.MockTransactionStart("issueIOU")
	...
	assert.Nil(t, err)
	key, _ := containerServiceStub.CreateCompositeKey(assetName, []string{"ae846b5b-2189-46b4-bda7-8511ff33ba6b_null"})
	value, err := containerServiceStub.GetState(key)
	assert.Nil(t, err)
	fmt.Printf("state=%v\n", string(value))
	containerServiceStub.MockTransactionStart("issueIOU")
}
func TestEvalArray(t *testing.T) {
	...
	// We assume there will be no errors
	assert.Nil(t, err)
	key, _ := containerServiceStub.CreateCompositeKey(assetName, []string{"ae846b5b-2189-46b4-bda7-8511ff33ba6b_1"})
	value, err := containerServiceStub.GetState(key)
	assert.Nil(t, err)
	fmt.Printf("state=%v\n", string(value))

	key, _ = containerServiceStub.CreateCompositeKey(assetName, []string{"ae846b5b-2189-46b4-bda7-8511ff33ba6b_2"})
	value, err = containerServiceStub.GetState(key)
	assert.Nil(t, err)
	fmt.Printf("state=%v\n", string(value))
	containerServiceStub.MockTransactionStart("issueIOU")
...
func put(containerServiceStub *shim.MockStub) {
	act := NewActivity(getActivityMetadata())
	tc := test.NewTestActivityContext(act.Metadata())
...
func putComposite(containerServiceStub *shim.MockStub) {
	act := NewActivity(getActivityMetadata())
	tc := test.NewTestActivityContext(act.Metadata())
