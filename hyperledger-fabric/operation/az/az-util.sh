...
# do not delete AD entry, if you do, you must also delete the local file ~/.azure/aksServicePrincipal.json
# if AD entry is deleted, cluster creation will fail next time when it uses the locally stored service principal
